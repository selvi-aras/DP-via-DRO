...
## values
ub_tlap = 1.22392 #from C++
lb_tlap = 0.573357 #from C++
ub_gauss = std_Gauss*sqrt(2)/sqrt(pi)
## try analytic gaussian
sigma = 1.6719974 #this is the value we need
...
for i in 1:nr_to_sample
    if isfile("./Results/Runtimes/Revised/mid_"*string(i)*".csv")
        push!(df, readdlm("./Results/Runtimes/Revised/mid_"*string(i)*".csv", ','))
    end
end
...
for i in 1:nr_to_sample
    file_name = "./Results/Runtimes/Revised/mid_m_"*string(i)*".csv"
    if isfile(file_name)
...
plot_fig= Plots.plot((df.row), df.lbOur, fillrange = df.ubOur,  legend=:right,
        legend_font = 9, fmt = :pdf, xaxis=(L"1/\beta"*"; "*L"K/4", font(12)), xguidefontsize=50, yaxis=("expected "*L"\ell_1"*"-loss", font(12)),
        fillalpha = 0.35,xlims = (1,nr_to_sample), xticks=(1:nr_to_sample, [1,2,4,8,16,32]),  ylims= (0.3, 1.42), yticks = 0.4:0.2:1.4, 
         c = 9, label = "Data Independent Noise", leftmargin = 4mm, rightmargin = 15mm, bottommargin = 4mm,topmargin = 5mm, dpi = 1000)
...
ax2 = twinx()
plot_fig = plot!(ax2, xguidefontsize=50, ylims = (0.5,10.5), yticks= 1:3:10, xlims = (1,nr_to_sample), xticks=-1:0, yaxis = ("runtime ("*L"\log(e + x)"*" seconds)", font(12)), linestyle = :dash, 
...
# Plots.savefig(plot_fig, "Images/mid_convergence_doublevertical.pdf")