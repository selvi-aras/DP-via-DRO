...
# #iterate over all files and report if results[8] > results[2]
# for job in 0:99
#     results = cpp(job)
#     if results[8] > results[2]
#         println(job)
#     end
# end
...
for job in job_nrs
    ...
    val1 = (tlap_ub - o)/(max(o,1))
    val2 = ((o-tlap_lb)/max(o,1))
    grey_scale = mean(grids .<= val1 + val2)/(2.5)
    ...
    string_tot = string_tot*"& "*"{\\cellcolor[gray]{"*string(round(1-grey_scale, digits = 3))*"} "*@sprintf("%.2f", (val1+val2)*100)*"\\%}"
