...
job_nr = 501 #parse(Int64, ENV["PBS_ARRAY_INDEX"])
...
noise_methods = ["naive", "gaussian", "analytic", "truncated", "optimal"]
# noise_methods = ["naive"]
nr_sim = 10^2
...
for method in noise_methods
    ...
    for sim in 1:nr_sim
        ...
        xi_collection = PCD_train(df_train, labels_train, sim, X, p, beta; epsilon = epsilon, delta = delta, lambda=10^-8, K = Int(ceil(d/4)), T = 100, method = method)
