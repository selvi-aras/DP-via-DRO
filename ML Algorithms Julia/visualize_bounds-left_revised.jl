...
## values
ub_tlap = 0.392281 #from C++
lb_tlap = 0.0136133 #from C++
diff_to_benchmark = ub_tlap - lb_tlap
ub_gauss = std_Gauss*sqrt(2)/sqrt(pi)
## try analytic gaussian
sigma = 0.5782 #this is the value we need
...
for i in 1:nr_to_sample
    if isfile("./Results/Runtimes/Revised/left_"*string(i)*".csv")
        push!(df, readdlm("./Results/Runtimes/Revised/left_"*string(i)*".csv", ','))
    end
end
...
for i in 1:nr_to_sample
    file_name = "./Results/Runtimes/Revised/m_"*string(i)*".csv"
    if isfile(file_name)
...
plot_fig= Plots.plot((df.row), df.lbOur, fillrange = df.ubOur,  legend=:inside,
        legend_font = 9, fmt = :pdf, xaxis=(L"1/\beta"*"; "*L"K/4", font(12)), xguidefontsize=50, yaxis=("expected "*L"\ell_1"*"-loss", font(12)),
        fillalpha = 0.35,xlims = (1,nr_to_sample), xticks=(1:nr_to_sample, [1,2,4,8,16,32]), 
...
ax2 = twinx()
plot_fig = plot!(ax2, xguidefontsize=50, xlims = (1,nr_to_sample), xticks=-1:0, yaxis = ("runtime ("*L"\log(e + x)"*" seconds)", font(12)), ylims = (0.4,10.5), yticks= 1:3:10, linestyle = :dash, 
...
plot_fig= plot!((df_mult.row), df_mult.lbOur, fillrange = df_mult.ubOur,
        fillalpha = 0.35, c = 13,legend = :none,  label = "Data Dependent Noise", dpi = 1000)
plot_fig= plot!((df_mult.row),df_mult.lbOur, msw = 0, ms = 1.5, c=13,legend = :none,  label ="")
plot_fig= plot!((df_mult.row),df_mult.ubOur, msw = 0, ms = 1.5, c=13,legend = :none,  label = "")
#now plot the runtime on the right-hand side
...
# Plots.savefig(plot_fig, "Images/left_convergence_doublevertical.pdf")
